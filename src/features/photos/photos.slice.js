  ...
  reducers: {
    ...
    // `splice()` documentation: https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/splice
      removePhoto: (state,action)=>{
        const location = state.photos.findIndex((photo)=> photo.id == action.payload)
        state.photos.splice(location,1);
      }
  },
};
...
export const selectFilteredPhotos = (state) => {
  ...
  //console.log(state.photos.photos.filter((photo) => photo.id == 1))
  return state.photos.photos.filter((photo) => photo.caption == state.search.searchTerm)
};
